...
export default function CioEditRopaPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5" }}>
      <header style={{ background: "#f57f17", color: "#fff", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={() => router.push(`/cio/approvals/${params.id}`)}
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 500 }}>แก้ไข ROPA (CIO) — {ropa.ropaId}</div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "1.5rem 1rem" }}>
        <div style={{ background: "#fff", borderRadius: 10, border: "0.5px solid #e0e0e0", padding: "1.5rem" }}>
          <RopaForm
            ...
            onSuccess={() => router.push(`/cio/approvals/${params.id}`)}
          />
        </div>
      </div>
    </div>
