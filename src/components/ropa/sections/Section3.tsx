...
export default function Section3({ data, onSave, onNext, onPrev }: SectionProps) {
  ...
  const toggle = (k: string) => {
    setSubjects(p => p.includes(k) ? p.filter(x => x !== k) : [...p, k])
    if (errors.subjects) setErrors(p => ({ ...p, subjects: "" }))
  }
  const handleNext = () => {
    if (subjects.length === 0) { setErrors({ subjects: "กรุณาเลือกประเภทเจ้าของข้อมูลอย่างน้อย 1 ประเภท" }); return }
    onSave({ dataSubjects: subjects, dataSubjectOther: otherText }); onNext?.()
  }
  return (
    <div>
      ...
      <div style={G.row}>
        ...
        <div style={{border: errors.subjects ? "1px solid #e53935" : "0.5px solid #e0e0e0", borderRadius:8, padding:"0.75rem 1rem", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 1rem"}}>
          {SUBJECTS.map(s => <Checkbox key={s.key} label={s.label} checked={subjects.includes(s.key)} onChange={() => toggle(s.key)} />)}
        </div>
        {errors.subjects && <div style={{fontSize:12, color:"#e53935", marginTop:4}}>{errors.subjects}</div>}
